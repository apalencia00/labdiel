...
$(document).ready(function(){
	...
	$("select#listClientelinears").change(function(){
		...
		$.ajax({
			...
			contentType : "application/json",
			dataType : "json",
			data : {"method" : 'getCotizacionCliente', "cliente" : $("#listClientelinears").val()},
			success: function(json)
			{
				...
				try{
					...
					html += '</table>';
					$('#act_table_coticlinears').html(html); 	
	...
	$(document).on("click", "#myclass3", function () {
		...
		$.ajax({
			...
			dataType : "json",
			contentType : "application/json",
			data : {"method" : 'getserialesEquiposLinears', 'cotizacion' :  codigo },
			...
			success : function(json){
				...
				try{
					...
					html += '</table>';
					$('#act_table_seriallinears').html(html);   
