...
$(document).ready(function(){
	...
	$.ajax({
		...
		success : function(json){
			...
			try{
				...
				var $select = $("#listClientepertigas");
	...
	$("select#listClientepertigas").change(function(){
		...
		$.ajax({
			...
			contentType : "application/json",
			dataType : "json",
			data : {"method" : 'getCotizacionCliente', "cliente" : $("#listClientepertigas").val()},
			success: function(json)
			{
				...
				try{
					...
					html += '</table>';
					$('#act_table_coticpertigas').html(html); 	
	...
	$(document).on("click", "#myclass3", function () {
		...
		$.ajax({
			...
			dataType : "json",
			contentType : "application/json",
			data : {"method" : 'getserialesEquiposPertigas', 'cotizacion' :  codigo },
			...
			success : function(json){
				...
				try{
					...
					html += '</table>';
					$('#act_table_serialpertigas').html(html);   
